  ...
  update: function(parameters) {
    if (Pandora.update_url) {
      if (/^https?:/.test(Pandora.update_url)) {
        window.location.href = Pandora.update_url +
          (Pandora.update_url.include('?') ? '&' : '?') +
...
Pandora.ElementMethods = {
  ...
  toggle_popup_disabled: function(element, checkbox_class) {
    ...
    if (element.id === 'image_list_form') {
      element.select_by_class_name('popup_toggle.disable').invoke(
        'toggle_class_name_if', !element.any_checked(checkbox_class), 'disabled'
      );
    }
  },
  ...
  autosubmit: function(element) {
    ...
    element.form_for_autosubmit(function(form) {
      ...
      if (!form.onsubmit || form.onsubmit()) {
        ...
        // them:
        var isCorrectForm = (
          (jQuery(form).attr('id') === 'image_list_form') ||
          (jQuery(form).attr('id') === 'upload_list_form')
        )
        if (isCorrectForm) {
