class Box < ApplicationRecord
  ...
  attr_protected :type, :params, :created_at, :updated_at,
    :owner_id, :image_id, :collection_id, :presentation_id

  validates_presence_of :type
  validates_presence_of :owner_id
  validates_presence_of :image_id,        :if => :image_box?
  validates_presence_of :collection_id,   :if => :collection_box?
  validates_presence_of :presentation_id, :if => :presentation_box?
  ...
  def self.category
    @category ||= name.underscore.sub!(/_box\z/, '')
  end
