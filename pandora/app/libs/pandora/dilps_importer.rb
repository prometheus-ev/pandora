  ...
  def import
    ...
    while record_count < records['hits']['total']['value']
      records['hits']['hits'].each do |record|
        ...
        record_count += 1
        Pandora.printf "\rInstitutional uploads created: #{record_count} (#{record['_source']['record_id']})" unless Rails.env.test?
      end
  ...
  private
    ...
    def create_institutional_upload(index_record)
      ...
      Tempfile.create(binmode: true) do |f|
        ...
        if !upload.save
          Pandora.puts ''
          Pandora.puts 'Could not save institutional upload.'
          Pandora.puts "Index record ID: #{index_record['record_id']}"
          Pandora.puts "Upload image ID: #{upload.image_id}"
          Pandora.puts upload.errors.full_messages.join("\n")
        end
      end
    end
end
