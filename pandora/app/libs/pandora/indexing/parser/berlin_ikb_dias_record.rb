class Pandora::Indexing::Parser::BerlinIkbDiasRecord < Pandora::Indexing::Parser::Record
  def record_id
  ...
  def date_range
    ...
    if !production_master_date.blank?
      @date_parser.date_range(production_master_date)
    elsif !production_slide_date.blank?
      @date_parser.date_range(production_slide_date)
    end
  end
  ...
  # A nested field is used for authority_files.
  # @see app/libs/indexing/index_mappings.rb 
  def authority_files
    authority_files = []
    labels = record.xpath(".//metadata/SUBJECT-Name/SUBJECT-Name/Authority-Files/Authority-File/link/text()")
    urls = record.xpath(".//metadata/SUBJECT-Name/SUBJECT-Name/Authority-Files/Authority-File/url/text()")

    return if urls.blank?
    return if labels.first.to_s == "Geodaten"

    urls.each_with_index do |url, index|
      authority_file = {}
      authority_file["label"] = CGI.unescapeHTML(labels[index].to_s)
      authority_file["id"] = url.to_s.split("/").last
