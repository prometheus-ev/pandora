class Indexing::Sources::Metropolitan < Indexing::SourceSuper
  ...
  def department
    compose_field(", ", object.xpath('./repository/text()').to_s, 
      object.xpath('./department/text()').to_s)
  end

  def classification
    compose_field(" | ", object.xpath('./classification/text()').to_s, 
  ...
  def epoch
    compose_field(", ", object.xpath('./period/text()').to_s, 
  ...
  def place
    compose_field(", ", object.xpath('./geographyType/text()').to_s, 
  ...
  def discoverplace
    compose_field(" | ", (compose_field(", ", object.xpath('./excavation/text()').to_s, 
