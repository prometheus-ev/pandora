...
class IndexTasksTest < ActiveSupport::TestCase
  ...
  test 'revert' do
    ...
    tasks.dump(['test_source'])

    assert_equal 'test_source_1', elastic.index_name_from(alias_name: 'test_source')
    assert elastic.index_exists?('test_source_1')
    assert_not elastic.index_exists?('test_source_2')

    tasks.load(['test_source'])
    assert_equal 'test_source_2', elastic.index_name_from(alias_name: 'test_source')
    assert elastic.index_exists?('test_source_1')
    assert elastic.index_exists?('test_source_2')
