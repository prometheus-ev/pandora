...
class MapquestRequestTest < ActionDispatch::IntegrationTest
  ...
  test 'test reverse request nominatim API response @brittle' do
  ...
  test 'test search request nominatim API response @brittle' do
  ...
  test 'test tile sector request API response @brittle' do
  ...
  test 'test tile request API response @skip' do
